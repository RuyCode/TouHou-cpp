...
bool CollisionManager::checkCollision(glm::vec2 aPos, glm::vec2 bPos, std::shared_ptr<Collider> a, std::shared_ptr<Collider> b) {
    if (typeid(*a) == typeid(CircleCollider) && typeid(*b) == typeid(CircleCollider)) {
        return checkCollision(aPos, bPos, std::dynamic_pointer_cast<CircleCollider>(a), std::dynamic_pointer_cast<CircleCollider>(b));
    } else if (typeid(*a) == typeid(CircleCollider) && typeid(*b) == typeid(RectangleCollider)) {
        return checkCollision(aPos, bPos, std::dynamic_pointer_cast<CircleCollider>(a), std::dynamic_pointer_cast<RectangleCollider>(b));
    } else if (typeid(*a) == typeid(RectangleCollider) && typeid(*b) == typeid(CircleCollider)) {
        return checkCollision(aPos, bPos, std::dynamic_pointer_cast<RectangleCollider>(a), std::dynamic_pointer_cast<CircleCollider>(b));
    } else if (typeid(*a) == typeid(RectangleCollider) && typeid(*b) == typeid(RectangleCollider)) {
        return checkCollision(aPos, bPos, std::dynamic_pointer_cast<RectangleCollider>(a), std::dynamic_pointer_cast<RectangleCollider>(b));
    }
    return false;
}
