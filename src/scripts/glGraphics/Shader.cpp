...
std::string LoadShader(const std::string& filename, std::unordered_set<std::string>& includedFiles) {
    Logger::Instance().Info("Including shader: ", filename);

    if (includedFiles.find(filename) != includedFiles.end()) {
        Logger::Instance().Error("Error: Circular include detected: ", filename);
        throw std::runtime_error("Error: Circular include detected: " + filename);
    }
    includedFiles.insert(filename);

    std::ifstream file(filename);
    if (!file) {
        Logger::Instance().Error("Could not open shader file: ", filename);
...
Shader::Shader(const std::string& vertexShaderFilePath, const std::string& fragmentShaderFilePath) {
    ...
    Logger::Instance().Info("Loaded shader program: ", vertexShaderFilePath, " and ", fragmentShaderFilePath, " with ID of ", ID);
}
...
Shader::~Shader() {
    Logger::Instance().Info("Deleted shader");
    Delete();
}
